...
#
# Module: Instagram Explorer & RapidAPI Hub (plugins/instagram.py)
# Description: Interactive Instagram browser (.ig), RapidAPI rotation and cloud DB
#              sync (.rapidapi), and cached profile/stories/posts/highlights fetcher.
# -----------------------------------------------------------------------------
...
def fetch_instagram_data(endpoint, username):
    ...
    if endpoint == "profile":
        ...
        if res.status_code != 200:
            if res.status_code in (401, 403):
                raise Exception("RapidAPI anahtarı yetkisiz! Lütfen 'instagram-looter2' API'sine abone olduğunuzdan emin olun.")
            elif res.status_code == 429:
                raise Exception("RapidAPI istek kotanız doldu! Yeni anahtar eklemek için: .rapidapi [yeni_anahtar]")
            raise Exception(f"Gönderiler çekilemedi (Kod {res.status_code}): {res.text[:120]}")
    ...
    elif endpoint == "stories":
        headers = {
            "x-rapidapi-key": get_rapidapi_headers(),
            "x-rapidapi-host": "instagram-best-experience.p.rapidapi.com",
        }
        res = requests.get("https://instagram-best-experience.p.rapidapi.com/stories", headers=headers, params={"user_id": user_id})
        if res.status_code != 200:
            if res.status_code in (401, 403):
                raise Exception("RapidAPI anahtarı yetkisiz! Lütfen 'instagram-best-experience' API'sine abone olduğunuzdan emin olun.")
            elif res.status_code == 429:
                raise Exception("RapidAPI istek kotanız doldu! Yeni anahtar eklemek için: .rapidapi [yeni_anahtar]")
            raise Exception(f"Hikayeler çekilemedi (Kod {res.status_code}): {res.text[:120]}")
        ...
        for item in data:
            ...
            stories_result.append(obj)
        return {"result": stories_result}
        ...
        res = requests.get("https://instagram-best-experience.p.rapidapi.com/highlights", headers=headers, params={"user_id": user_id})
        if res.status_code != 200:
            if res.status_code in (401, 403):
                raise Exception("RapidAPI anahtarı yetkisiz! Lütfen 'instagram-best-experience' API'sine abone olduğunuzdan emin olun.")
            elif res.status_code == 429:
                raise Exception("RapidAPI istek kotanız doldu! Yeni anahtar eklemek için: .rapidapi [yeni_anahtar]")
            raise Exception(f"Öne çıkanlar çekilemedi (Kod {res.status_code}): {res.text[:120]}")
        ...
        for item in data:
            ...
            if item.get("media_type") == 2:
                ...
                obj["image_versions2"] = item.get("image_versions2", {})
            hl_result.append(obj)
        return {"result": hl_result}
