...
# Component: Dynamic Plugin Manager & Store Client
...
ggr_builtin_plugins = {
    ...
    "sohbet", "tiktok", "plugin_manager", "afk", "rehber", "stt",
    "tools", "settings", "tagger", "broadcast", "protection",
    "media", "automessage", "contacts", "system"
}
...
async def _resolve_store_target_msg(client, target_num):
    """Mağaza kanalından belirtilen numaraya ait mesajı arar."""
    try:
    ...
    if 1 <= target_num <= len(store_plugins):
        ...
        return await client.get_messages(ggr_plugin_store_channel, message_ids=item["id"])
...
async def cmd_install(client, message):
    ...
    if message.reply_to_message and message.reply_to_message.document:
        await _install_from_reply(client, message, message.reply_to_message.document)
        return

    if args and args[0].isdigit():
        await _install_from_store(client, message, int(args[0]))
        return
