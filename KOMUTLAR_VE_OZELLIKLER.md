...
### Kalıcı Tek Bulut Veritabanı
* Admin grup ID'si, aktif ayarlar ve TikTok takip listeleri Telegram Kayıtlı Mesajlar (`Saved Messages`) içinde `#GAGARAGOGO_TEK_BULUT_DB` etiketli tek bir mesajda saklanır.
* Değişikliklerde mesaj silinmez; `edit_text` ile güncellenir.
* Sunucu yeniden kurulsa ya da geçici disk sıfırlansa bile bot mevcut grubu bu mesajdan okur ve yeni grup açmaz.
...
### Toplu İletim ve Zamanlama (Broadcast & Otomesaj)
